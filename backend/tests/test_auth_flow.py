...
class AuthFlowTests(unittest.TestCase):
    ...
    def test_competitor_product_discovery_endpoint_returns_candidates_and_links(self) -> None:
        ...
        self.assertEqual({item["id"] for item in payload["items"]}, {"cand_restore"})
        ...
        store77_summary = next(item for item in payload["source_summaries"] if item["source_id"] == "store77")
        self.assertEqual(store77_summary["status"], "confirmed")
        self.assertEqual(store77_summary["confirmed_count"], 1)
    ...
    def test_competitor_product_discovery_endpoint_hides_low_confidence_review_items(self) -> None:
        ...
        self.assertEqual(restore_summary["hidden_count"], 1)
        self.assertEqual(restore_summary["best_title"], "Apple Watch Series 11")
    ...
    def test_competitor_product_discovery_source_summary_uses_scan_state_without_candidate(self) -> None:
        ...
        scan_row = {
            ...
            "payload": {
                ...
                "candidates_count": 0,
                "evidence": {
                    "direct_url": "https://re-store.ru/catalog/10117E256PNKN/",
                    "query_terms": ["Apple iPhone 17e 256Gb Pink"],
                },
            },
        }
    ...
    def test_competitor_candidate_moderation_reads_relational_candidate_without_json(self) -> None:
        ...
        store = {
            ...
            "categories": {},
            "templates": {},
            "discovery": {"candidates": {}, "links": {}, "runs": {}},
        }
        saved_docs: list[dict] = []
        channel_links: list[dict] = []
        relational_candidate = {
            "link_id": "cand_rel_restore",
            "entity_id": "product_1",
            "provider": "restore",
            "url": "https://re-store.ru/product/iphone",
            "title": "iPhone 16 Pro",
        ...
        with (
            patch.object(competitor_mapping_routes, "load_competitor_mapping_db", return_value=store),
            patch.object(competitor_mapping_routes, "save_competitor_mapping_db", side_effect=lambda doc: saved_docs.append(deepcopy(doc))),
            patch.object(competitor_mapping_routes, "list_pim_channel_links", return_value=[relational_candidate]),
            patch.object(competitor_mapping_routes, "upsert_pim_channel_link", side_effect=lambda row: channel_links.append(deepcopy(row)) or row),
        ):
            response = self.client.post(
                "/api/competitor-mapping/discovery/candidates/cand_rel_restore/moderate",
                json={"action": "approve"},
            )
    ...
    def test_variant_profile_keeps_iphone_17e_separate_from_iphone_17_esim(self) -> None:
        iphone_17e = competitor_mapping_routes._variant_profile("Apple iPhone 17e 256GB, Soft Pink")
        iphone_17_esim = competitor_mapping_routes._variant_profile("Apple iPhone 17 eSIM 256GB, Lavender")

        self.assertEqual(iphone_17e["model"], "iphone_17_e")
        self.assertEqual(iphone_17e["color"], "pink")
        self.assertEqual(iphone_17_esim["model"], "iphone_17")
        self.assertEqual(iphone_17_esim["sim"], "esim_only")

    def test_restore_direct_url_builds_iphone_17e_pink_url(self) -> None:
        product = {
            "id": "product_17e",
            "title": "Смартфон Apple iPhone 17e 256Gb nano SIM+eSIM Pink (Global)",
            "sku_gt": "52517",
        }
    ...
    def test_restore_search_rejects_iphone_17_esim_for_iphone_17e(self) -> None:
        html = r'''
          <script>
          window.__payload = {\"linkedProducts\":[
          {\"skuCode\":\"10117e256PNKn\",\"brandName\":\"Apple\",\"name\":\"Apple iPhone 17e 256GB, Soft Pink\",\"link\":\"/catalog/10117E256PNKN/\"},
          {\"skuCode\":\"AG_10117256LVRe\",\"brandName\":\"Apple\",\"name\":\"Apple iPhone 17 eSIM 256GB, Lavender\",\"link\":\"/catalog/AG_10117256LVRE/\"}
          ]};
          </script>
        '''
        product = {
        ...
        self.assertEqual([item["url"] for item in candidates], ["https://re-store.ru/catalog/10117E256PNKN/"])
        self.assertGreaterEqual(candidates[0]["confidence_score"], 0.78)
