    ...
    try:
        ...
        if not isinstance(job, dict):
            return {
                ...
                "skipped": True,
                "job_id": job_id,
                "reason": "AI matching job is not queued or was already claimed.",
            }
        catalog_category_id = str(job.get("catalog_category_id") or "").strip()
        if not catalog_category_id:
            raise RuntimeError(f"AI matching job has no catalog_category_id: {job_id}")
        req = marketplace_mapping.AiMatchReq(apply=bool(job.get("apply", True)))
        await marketplace_mapping._run_attr_ai_match_job(job_id, catalog_category_id, req)
        return {
            "ok": True,
            "job_id": job_id,
        ...
        for job in jobs:
            ...
            try:
                ...
                if isinstance(result, dict) and result.get("skipped"):
                    ...
                    failed += 1
                    errors.append({"job_id": job_id, "error": str(result.get("reason") or "AI_MATCH_JOB_FAILED")[:500]})
                else:
                    completed += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one marketplace attribute AI matching job.")
