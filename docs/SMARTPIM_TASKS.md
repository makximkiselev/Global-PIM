...
3. Frontend table foundation:
   ...
   Status: in progress. TanStack Table and TanStack Virtual are installed. The main product registry table and export queue/payload tables now use TanStack Table column/header/cell rendering while preserving the existing table DOM and CSS. The value-mapping and parameter-mapping field lists now use TanStack Table row identity plus TanStack Virtual rendering so dense categories do not mount every field card at once. Remaining work: apply table/virtual primitives to any smaller dense review queues as they are touched.
...
14. Product/category workflow context now uses a shared frontend helper (`productFlowContext`) instead of duplicated localStorage reads/writes in product workspace and `/sources`, so the next-action state has one stable shape.
15. Internal CTA links inside `/sources` params/value workspaces now preserve `product=` when the user entered from a SKU, so `Проверить SKU`, `К параметрам`, `Настроить значения`, and source back-links do not silently switch to category export.
...
19. Product queue actions now preserve SKU context when opening `/sources`: `Собрать`, `Сопоставить`, inspector `Открыть сопоставления`, and bulk `К сопоставлениям` pass category plus the relevant product id.
...
25. Export preparation also fills missing media/description/brand/package logistics from sibling variants with the same category, model line, and color. This covers line variants such as `eSIM`, `Sim+eSIM`, and `2Sim` where only the SIM axis differs; package fields are copied only into empty PIM fields and keep `variant_sibling` lineage.
