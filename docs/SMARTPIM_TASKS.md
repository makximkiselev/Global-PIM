...
19. remove JSON as an active source of truth for competitor workflow. Status: active 2026-05-12; candidate/link writes now upsert into `pim_channel_links`, product/category/candidates API reads merge `pim_channel_links`, moderation can approve/reject candidates that exist only in `pim_channel_links`, and backend tests cover moderation/manual-link/product-context paths. Production candidate backfill completed 2026-05-12 with `87` competitor product rows (`candidate=68`, `stale=16`, `rejected=2`, `confirmed=1`; `restore=55`, `store77=32`). Discovery run, approve/reject, manual link, and enrichment no longer persist candidates/links into JSON; `_save_competitor_mapping_runs_only` backfills any in-memory legacy candidate/link rows to `pim_channel_links` before clearing them from JSON. Category/template competitor mappings now also read/write through `pim_channel_links` with scope `competitor_mapping`, and `backend/scripts/migrate_competitor_mappings_to_channel_links.py` backfills legacy JSON rows. Production mapping backfill completed 2026-05-12 with `12` rows (`template=9`, `category=3`, providers `restore=4`, `store77=4`, `__meta=4`, configured `11`). JSON remains only for discovery run polling/state and fallback until production parity is verified.
