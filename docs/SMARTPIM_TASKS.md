...
6. consolidated target tables are still partial: `pim_channel_links` now covers competitor product/category evidence and `pim_workflow_runs` covers competitor discovery state, while `pim_products`, `pim_model_fields`, and `pim_external_payloads` remain pending; the current schema is still split across legacy/tenant/read-model tables.
...
1. fix QA findings from 2026-05-09 full pipeline pass for `Смартфоны` and `Наушники`:
   ...
   - competitor data still had an active JSON discovery path in `json_documents` (`competitor_mapping_org_default.json`). Status: active 2026-05-12; `pim_channel_links` is now the relational store for competitor SKU candidates, confirmed links, rejected links, stale candidates, moderation evidence, and category/template competitor mappings. `pim_workflow_runs` is now the relational store for discovery run polling/state. Remaining JSON work: deploy/backfill old discovery runs and then keep JSON only as read-only migration fallback.
...
19. remove JSON as an active source of truth for competitor workflow. Status: done 2026-05-12; candidate/link writes now upsert into `pim_channel_links`, product/category/candidates API reads merge `pim_channel_links`, moderation can approve/reject candidates that exist only in `pim_channel_links`, and backend tests cover moderation/manual-link/product-context paths. Production candidate backfill/clear completed 2026-05-12; current `pim_channel_links` competitor product rows are `candidate=67`, `stale=17`, `rejected=2`, `confirmed=1`. Discovery run, approve/reject, manual link, and enrichment no longer persist candidates/links into JSON; `_save_competitor_mapping_runs_only` backfills any in-memory legacy candidate/link rows to `pim_channel_links` before clearing them from JSON. Category/template competitor mappings now also read/write through `pim_channel_links` with scope `competitor_mapping`, and production has `category=3`, `template=9` rows. Discovery run polling/state writes to `pim_workflow_runs`; production has `completed=40`, `queued=7`, `running=7` competitor discovery run rows. Verified production JSON cleanup 2026-05-12: `competitor_mapping.json` and `competitor_mapping_org_default.json` both have `categories=0`, `templates=0`, `candidates=0`, `links=0`, `runs=0`.
...
4. Current consolidated target tables are partially implemented: `pim_channel_links` now stores competitor product/category evidence and `pim_workflow_runs` stores competitor discovery run state; `pim_products`, `pim_model_fields`, and `pim_external_payloads` remain pending.
...
6. Competitor source workflow is close to DB-complete: `competitor_mapping_org_default.json` still exists in `json_documents` as compatibility/migration fallback, while product-level candidates/links and category/template competitor bindings use `pim_channel_links`; legacy run logs now target `pim_workflow_runs` and need production backfill verification.
