  ...
  return (
    <div className="pn-wrap pn-page pnWizardPage">
      ...
      <CatalogProductPickerModal
        ...
        treeById={treeById}
        flatById={flatById}
        initialSelected={pickerMode === "analogs" ? analogs.map((a) => ({ id: a.sku, name: a.name })) : related.map((r) => ({ id: r.sku, name: r.name }))}
        onApply={(items) => {
          if (pickerMode === "analogs") setAnalogs(items.map((i) => ({ sku: i.id, name: i.name })));
          else if (pickerMode === "related") setRelated(items.map((i) => ({ sku: i.id, name: i.name })));
        }}
