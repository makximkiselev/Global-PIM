  ...
  if (flowBlockerTotal > 0) {
    ...
    return {
      ...
      detail: [
        ...
        pimTasks ? `быстрая очередь: ${pimTasks} пустых характеристик PIM` : "",
  ...
  if (!hasInfoModel) {
    return (
      <EmptyState
        ...
        description={
          rawFeatureCount
            ? `В товаре сохранено ${rawFeatureCount} сырых параметров из источников, но они не считаются характеристиками PIM до создания инфо-модели категории.`
            : "Сначала соберите и подтвердите инфо-модель категории, после этого здесь появятся характеристики PIM для выгрузки."
        }
      />
    );
  }
